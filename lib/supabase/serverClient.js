...
export function getSupabaseClientSafe(useServiceRole = false) {
  ...
  if (!url || !key) {
    console.error('[Supabase] Falha ao criar cliente:', {
      url: url ? 'configurado' : 'FALTANDO',
      key: key ? 'configurado' : 'FALTANDO',
      useServiceRole
    });
    return null;
  }

  // Log para debug - mostra qual tipo de key está sendo usado
  console.log('[Supabase] Cliente criado:', {
    tipo: useServiceRole ? 'SERVICE_ROLE' : 'ANON',
    serviceRoleKey: useServiceRole && process.env.SUPABASE_SERVICE_ROLE_KEY ?
      'CONFIGURADO' :
      useServiceRole ? '❌ FALTANDO' : 'não usado'
  });
