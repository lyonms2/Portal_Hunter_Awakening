...
export function usarHabilidade(atacante, habilidade, defensor, estado) {
  ...
  if (habilidade.tipo === 'Defensiva' && habilidade.dano_base === 0) {
    ...
    let mensagem = `${atacante.nome} usou ${habilidade.nome}! Causou ${dano} de dano${critico ? ' (CRÍTICO!)' : ''}${vantagemMsg}`;
    if (curaRouboVida > 0) {
...
export function iniciarTurno(avatar, estado) {
  ...
  // Processar buffs/debuffs ativos
  if (avatar.buffs && avatar.buffs.length > 0) {
  ...
  if (avatar.debuffs && avatar.debuffs.length > 0) {
    ...
    avatar.debuffs.forEach(debuff => {
      ...
      if (debuff.tipo === 'cura_continua' && debuff.cura_por_turno) {
        const cura = Math.floor(avatar.hp_maximo * debuff.cura_por_turno);
...
export function verificarVitoria(estado) {
  // HITKILL: Ao invés de deixar HP em 0, deixa em 1
  if (estado.jogador.hp_atual <= 0) {
    estado.jogador.hp_atual = 1;
  }
  if (estado.inimigo.hp_atual <= 0) {
    estado.inimigo.hp_atual = 1;
  }

  const jogadorMorto = estado.jogador.hp_atual <= 1 && estado.jogador.hp_maximo > 1;
  const inimigoMorto = estado.inimigo.hp_atual <= 1 && estado.inimigo.hp_maximo > 1;
