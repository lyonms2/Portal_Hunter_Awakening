...
export const CONFIG_BATALHA = {
  ENERGIA_INICIAL: 100,
  ENERGIA_MAXIMA: 100,
  ENERGIA_POR_RECARREGAR: 35,      // Ação de recarregar
  ENERGIA_POR_DEFENDER: 20,        // Defender também recupera
  ENERGIA_POR_ATAQUE_BASICO: 10,   // Ataque básico recupera um pouco
  RODADAS_MAXIMAS: 20,
...
export function ataqueBasico(atacante, defensor, estado) {
  ...
  // Ataque básico recupera energia
  const energiaRecuperada = CONFIG_BATALHA.ENERGIA_POR_ATAQUE_BASICO;
  resultado.novaEnergia = Math.min(CONFIG_BATALHA.ENERGIA_MAXIMA, atacante.energia_atual + energiaRecuperada);
  resultado.energiaRecuperada = energiaRecuperada;

  resultado.mensagem = `🎲 ${d20}+${bonusFoco} = ${rolagemTotal} | ${atacante.nome} usou Ataque Básico! ⚔️ ${dano} de dano${criticoMsg}${vantagemMsg} ⚡+${energiaRecuperada}`;

  return resultado;
}

/**
 * Ação de Recarregar - recupera energia significativa
 */
export function recarregar(avatar) {
  const energiaRecuperada = CONFIG_BATALHA.ENERGIA_POR_RECARREGAR;
  ...
  return {
    sucesso: true,
    mensagem: `⚡ ${avatar.nome} está recarregando energia! +${energiaReal} energia`,
