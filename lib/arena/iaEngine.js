...
import { usarHabilidade, defender, recarregar, calcularDano, ataqueBasico } from './batalhaEngine';
...
const COMPORTAMENTOS = {
  facil: {
    ...
    usa_recarregar: 0.10,   // Recarrega menos (era 0.20)
  ...
  normal: {
    ...
    usa_recarregar: 0.08,   // Recarrega menos (era 0.15)
  ...
  dificil: {
    ...
    usa_recarregar: 0.07,   // Quase nunca recarrega (era 0.12)
    prioriza_ultimate: true,
    considera_vantagem_elemental: true,
    considera_hp_baixo: true,
  ...
  mestre: {
    ...
    usa_recarregar: 0.05,   // Quase nunca recarrega (era 0.10)
    prioriza_ultimate: true,
    considera_vantagem_elemental: true,
    considera_hp_baixo: true,
...
export function decidirAcaoIA(estado) {
  ...
  // Recarregar?
  if (rand < comportamento.usa_defesa + comportamento.usa_recarregar) {
    return { tipo: 'recarregar' };
  }
...
export function processarTurnoIA(estado) {
  ...
  switch (acao.tipo) {
    ...
    case 'recarregar':
      resultado = recarregar(estado.inimigo);
      estado.inimigo.energia_atual = resultado.novaEnergia;
      break;
...
export function getMensagemIA(acao, estado) {
  const mensagens = {
    ...
    recarregar: [
      "O adversário está recarregando energia! ⚡",
      "Seu oponente concentra suas forças!",
      "Uma aura de energia envolve o inimigo...",
    ]
  };
