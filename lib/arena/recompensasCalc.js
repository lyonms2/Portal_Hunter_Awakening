...
export function calcularRecompensasTreino(estado, vencedor) {
  ...
  if (vencedor !== 'jogador') {
    ...
    recompensas.exaustao = Math.floor(base.exaustao * 1.5); // 150% da exaustão (MAIOR ao perder!)
    recompensas.vinculo = -2; // Perde 2 pontos de vínculo na derrota
    recompensas.mensagens.push('☠️ Derrota... seu avatar sofreu mais exaustão!');
    recompensas.mensagens.push('💔 Seu avatar ficou muito desapontado (-2 Vínculo)');
