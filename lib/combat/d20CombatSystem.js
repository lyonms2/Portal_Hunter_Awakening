 ...
 */
export function inicializarBatalhaD20(avatarJogador, avatarInimigo) {
  const hpJogador = calcularHP(avatarJogador);
  const hpInimigo = calcularHP(avatarInimigo);
  ...
  return {
    ...
    jogador: {
      ...
      bonus_vinculo: calcularBonusVinculo(avatarJogador.vinculo)
    },
    ...
    inimigo: {
      ...
      hp_maximo: hpInimigo,
      hp_atual: hpInimigo,
      defesa: calcularDefesa(avatarInimigo),
      bonus_vinculo: calcularBonusVinculo(avatarInimigo.vinculo || 50) // IA tem vínculo médio
    },
 ...
 * @param {string} acaoInimigo - Ação da IA
 * @returns {Object} Resultado do turno
 */
export function processarTurnoD20(estado, acaoJogador, acaoInimigo = null) {
  const logs = [];
  let resultado = {
    jogadorDano: 0,
    inimigoDano: 0,
  ...
  // === TURNO DO JOGADOR ===
  logs.push(`\n🔵 TURNO DO JOGADOR (${estado.jogador.nome})`);
  ...
  // === TURNO DO INIMIGO (IA) ===
  logs.push(`\n🔴 TURNO DO INIMIGO (${estado.inimigo.nome})`);

  // IA escolhe ação (simples: sempre ataca)
  ...
  if (ataqueInimigo.acertou) {
    // Jogador pode ter escolhido esquivar ou defender
  ...
  if (estado.rodada >= CONFIG_D20.RODADAS_MAXIMAS && !resultado.fimBatalha) {
    logs.push(`\n⏱️ Tempo esgotado! Batalha termina por limite de rodadas.`);
    resultado.fimBatalha = true;
