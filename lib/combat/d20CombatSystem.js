 ...
 * Avatares com 100% de exaustão não podem lutar
 */
export const PENALIDADE_EXAUSTAO = {
  LIMITE_LUTA: 100, // 100% = não pode lutar
  FAIXAS: [
    { min: 0, max: 39, penalidade: 0 },      // Normal
    { min: 40, max: 59, penalidade: 0.05 },  // -5% stats
    { min: 60, max: 79, penalidade: 0.15 },  // -15% stats
    { min: 80, max: 99, penalidade: 0.25 },  // -25% stats
    { min: 100, max: 100, penalidade: 1.0 }  // Não pode lutar
  ]
};

/**
 * Calcula penalidade de exaustão
 * @param {number} exaustao - Nível de exaustão (0-100)
 * @returns {number} Penalidade (0 a 1)
 */
export function calcularPenalidadeExaustao(exaustao) {
  const nivel = exaustao || 0;
  const faixa = PENALIDADE_EXAUSTAO.FAIXAS.find(f => nivel >= f.min && nivel <= f.max);
  return faixa ? faixa.penalidade : 0;
}

/**
 * Aplica penalidade de exaustão aos stats
 * @param {Object} avatar - Avatar com stats
 * @returns {Object} Stats ajustados com penalidade
 */
export function aplicarExaustaoStats(avatar) {
  const penalidade = calcularPenalidadeExaustao(avatar.exaustao);
  const multiplicador = 1 - penalidade;

  return {
    forca: Math.floor((avatar.forca || 10) * multiplicador),
    agilidade: Math.floor((avatar.agilidade || 10) * multiplicador),
    resistencia: Math.floor((avatar.resistencia || 10) * multiplicador),
    foco: Math.floor((avatar.foco || 10) * multiplicador)
  };
}

/**
 * Verifica se avatar pode lutar
 * @param {Object} avatar
 * @returns {Object} { podeLutar: boolean, motivo: string }
 */
export function verificarPodeLutar(avatar) {
  if (!avatar.vivo) {
    return { podeLutar: false, motivo: 'Avatar está morto' };
  }

  if ((avatar.exaustao || 0) >= PENALIDADE_EXAUSTAO.LIMITE_LUTA) {
    return { podeLutar: false, motivo: 'Avatar está exausto demais para lutar (100% exaustão)' };
  }

  return { podeLutar: true, motivo: null };
}
...
export function inicializarBatalhaD20(avatarJogador, avatarInimigo, dificuldade = 'normal') {
  ...
  // Aplicar penalidade de exaustão ao jogador
  const statsJogadorAjustados = aplicarExaustaoStats(avatarJogador);
  const jogadorComExaustao = {
    ...avatarJogador,
    ...statsJogadorAjustados,
    stats_originais: {
  ...
  const hpJogador = calcularHP(jogadorComExaustao);
  const hpInimigo = calcularHP(inimigoAjustado);

  const penalidadeExaustao = calcularPenalidadeExaustao(avatarJogador.exaustao);

  return {
    id: `battle_d20_${Date.now()}`,
    rodada: 1,
    dificuldade,
    exaustao_jogador: avatarJogador.exaustao || 0,
    penalidade_exaustao: penalidadeExaustao,

    jogador: {
      ...jogadorComExaustao,
      hp_maximo: hpJogador,
      hp_atual: hpJogador,
      defesa: calcularDefesa(jogadorComExaustao),
...
export function processarTurnoD20(estado, acaoJogador) {
  ...
  if (resultado.inimigoHP <= 0) {
    ...
    const novoEstado = {
      ...
      jogador: {
        ...estado.jogador,
        hp_atual: resultado.jogadorHP
      },
      inimigo: {
        ...estado.inimigo,
        hp_atual: resultado.inimigoHP
      }
    };
    ...
    return {
      ...
      novoEstado,
      ataqueJogador: {
        acertou: resultado.jogadorAcertou,
        critico: resultado.jogadorCritico,
        danoFinal: resultado.inimigoDano
      },
      ataqueInimigo: {
        acertou: false,
        critico: false,
  ...
  return {
    ...
    ataqueInimigo: {
      acertou: resultado.inimigoAcertou,
      critico: resultado.inimigoCritico,
      danoFinal: resultado.jogadorDano
    }
  };
}
...
export default {
  ...
  calcularDefesa,
  calcularHP,
  calcularPenalidadeExaustao,
  aplicarExaustaoStats,
  verificarPodeLutar,
  realizarAtaque,
  tentarEsquiva,
  tentarDefesa,
